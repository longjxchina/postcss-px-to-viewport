...
module.exports = postcss.plugin("postcss-px-to-viewport", function (options) {
  ...
  return function (css, result) {
    ...
    css.walkRules(function (rule) {
      ...
      const pcRule = rule.clone().removeAll();
    ...
    if (pcRules.length > 0) {
      var pcRoot = new postcss.atRule({
        params: `(min-device-width:${opts.viewportWidth}px)`,
        name: "media",
      });
      pcRules.forEach((rule) => pcRoot.append(rule));
      css.append(pcRoot);
    }
  };
});
